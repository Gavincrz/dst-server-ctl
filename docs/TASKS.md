...
- 安装任务目前只有任务状态，没有 stdout/stderr 落盘或查询入口；真实 SteamCMD/DST 安装耗时较长时，排查粒度还不够。
