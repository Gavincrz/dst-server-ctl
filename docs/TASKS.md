...
当前项目已有 harness、工程骨架、managed root 路径布局、共享 command runner、SQLite 状态存储基础层、启动时 managed root 初始化、安装状态 API、安装任务 API、任务模型、由任务驱动的 SteamCMD/DST 安装执行流程，以及围绕首次安装的初始化状态页。当前交互已弱化任务术语，改为突出初始化是否完成、当前安装步骤和失败重试入口。

## 下一任务

- [ ] 为安装失败场景补更明确的重试引导，并视情况补充最小历史视图。

## 后续任务

- [ ] 评估初始化页是否继续保留完整任务历史，还是收敛为最近一次安装摘要。
...
- 初始化页里是否要继续保留完整任务历史，还是只保留最近一次安装摘要。
