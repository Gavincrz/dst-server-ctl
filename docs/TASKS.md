...
- 当前更新日志入口只覆盖 `update_dst` 任务；`POST /api/v1/update/check` 和定时检查仍然直接返回状态，没有单独的 stdout/stderr 落盘可供排查。
