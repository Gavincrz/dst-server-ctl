...
- 每个任务都要保持代码健康不下降；如果能顺手减少重复、澄清边界或抽取公共逻辑，优先做这类小步整理。
- 优先复用、合并或抽取逻辑，不要复制相似代码后再局部修改。
- 如果一个需求只能通过跨层耦合或补丁式分支实现，先做小步重构，再继续功能改动。
- 行为变更和纯重构能拆就拆；较大清理机会先记录到 `docs/TASKS.md`，不要无边界混入当前任务。
- 非平凡重构前先补最小测试安全网；如果做不到，结束时要明确说明风险。
